...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIT, (float, x, x)(float, y, y)(float, z, z)(
...
LandmarkData ToLandmarkData(const LandmarkList& landmark_list) {
    ...
    for (const LandmarkEntry& entry : landmark_list.landmarks) {
        landmark_data.landmark_observations.push_back(
            {entry.id, ToRigid3d(entry.tracking_from_landmark_transform), entry.translation_weight,
             entry.rotation_weight});
    }
 ...
 */
cartographer::transform::Rigid3d ComputeLocalFrameFromLatLong(const double latitude,
...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
    const cartographer::io::PaintSubmapSlicesResult& painted_slices, const double resolution,
