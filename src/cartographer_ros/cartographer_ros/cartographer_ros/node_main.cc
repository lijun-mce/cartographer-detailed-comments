 ...
 * 定义参数通过DEFINE_type宏实现, 该宏的三个参数含义分别为命令行参数名, 参数默认值,
 * 以及参数的帮助信息 当参数被定义后, 通过FLAGS_name就可访问到对应的参数
 */
...
void Run() {
    ...
    // node_options 和 trajectory_options 对应参数文件是backpack_2d.lua
    ...
    // 从一个对象转移到另一个对象, 这样能够减少不必要的临时对象的创建、拷贝以及销毁, 能够大幅度提高
    // C++ 应用程序的性能. 临时对象的维护 ( 创建和销毁 ) 对性能有严重影响.

    // node对象控制着系统的业务逻辑，对象map_builder则用于完成建图
    // Node类的初始化
    // 参数、  map_builder、  tf、   false (node的构造函数中构造了 map_builder_bridge )
    Node node(node_options, std::move(map_builder), &tf_buffer, FLAGS_collect_metrics);

    // 如果load_state_filename非空  则加载pbstream文件 目的去初始化一些数据？（猜测）
