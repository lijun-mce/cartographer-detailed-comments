...
/**
 * @brief SensorBridge 构造函数, 并且初始化TfBridge
 * @param[in] num_subdivisions_per_laser_scan 激光数据分段数量
 * @param[in] tracking_frame 数据都转换到tracking_frame 参考坐标系
 * @param[in] lookup_transform_timeout_sec 查找tf的超时时间
 * @param[in] tf_buffer tf_buffer  tf坐标缓存
 * @param[in] trajectory_builder 轨迹构建器
 */
SensorBridge::SensorBridge(const int num_subdivisions_per_laser_scan,
...
void SensorBridge::HandleOdometryMessage(const std::string& sensor_id,
    ...
    if (odometry_data != nullptr) {
        ...
        trajectory_builder_->AddSensorData(
            sensor_id, carto::sensor::OdometryData{odometry_data->time, odometry_data->pose});
    }
}
...
void SensorBridge::HandleNavSatFixMessage(const std::string& sensor_id,
    ...
    if (msg->status.status == sensor_msgs::NavSatStatus::STATUS_NO_FIX) {
        trajectory_builder_->AddSensorData(
            sensor_id, carto::sensor::FixedFramePoseData{time, absl::optional<Rigid3d>()});
        return;
    }
    ...
    if (!ecef_to_local_frame_.has_value()) {
        ...
        LOG(INFO) << "Using NavSatFix. Setting ecef_to_local_frame with lat = " << msg->latitude
                  << ", long = " << msg->longitude << ".";
    }
    ...
    trajectory_builder_->AddSensorData(
        sensor_id, carto::sensor::FixedFramePoseData{
                       time, absl::optional<Rigid3d>(Rigid3d::Translation(
                                 ...
                                 LatLongAltToEcef(msg->latitude, msg->longitude, msg->altitude)))});
}
...
void SensorBridge::HandleLandmarkMessage(const std::string& sensor_id,
    ...
    auto tracking_from_landmark_sensor =
        tf_bridge_.LookupToTracking(landmark_data.time, CheckNoLeadingSlash(msg->header.frame_id));
...
/**
 * @brief rosmsg of sensor_msgs/Imu
        Header header
        geometry_msgs/Quaternion orientation
        float64[9] orientation_covariance  机器人姿态协方差矩阵
        geometry_msgs/Vector3 angular_velocity  角速度
        float64[9] angular_velocity_covariance  协方差
        geometry_msgs/Vector3 linear_acceleration  线加速度
        float64[9] linear_acceleration_covariance  协方差
*/
// 进行数据类型转换与坐标系的转换
std::unique_ptr<carto::sensor::ImuData> SensorBridge::ToImuData(
    const sensor_msgs::Imu::ConstPtr& msg) {
    // 检查是否存在线性加速度与角速度 协方差中第一个元素为-1,证明该数据无效
...
/**
 * @brief msg of sensor_msgs/MultiEchoLaserScan
        ...
        float32 range_min
        float32 range_max
        sensor_msgs/LaserEcho[] ranges  扫描距离值
        sensor_msgs/LaserEcho[] intensities  数据强度 和 ranges 一一对应
*/
// 处理MultiEchoLaserScan数据, 先转成点云,再传入trajectory_builder_
void SensorBridge::HandleMultiEchoLaserScanMessage(
    const std::string& sensor_id, const sensor_msgs::MultiEchoLaserScan::ConstPtr& msg) {
    // sensor_msgs::MultiEchoLaserScan 为ros系统中对应的消息类型 详细信息可以bing
    carto::sensor::PointCloudWithIntensities point_cloud;
    carto::common::Time time;
    // 将ROS的消息转换成点云数据 ms_conversion.cc中定义
    std::tie(point_cloud, time) = ToPointCloudWithIntensities(*msg);
    HandleLaserScan(sensor_id, time, msg->header.frame_id, point_cloud);
}
...
void SensorBridge::HandleLaserScan(const std::string& sensor_id, const carto::common::Time time,
    ...
    for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
        ...
        if (it != sensor_to_previous_subdivision_time_.end() &&
            ...
            LOG(WARNING) << "Ignored subdivision of a LaserScan message from sensor " << sensor_id
                         << " because previous subdivision time " << it->second
                         << " is not before current subdivision time " << subdivision_time;
            continue;
        }
        // 更新对应sensor_id的时间戳
...
void SensorBridge::HandleRangefinder(const std::string& sensor_id, const carto::common::Time time,
    ...
    if (sensor_to_tracking != nullptr) {
        // trajectory_builder是由map_builder提供的一个对象
        trajectory_builder_->AddSensorData(
