...
/**
 * @brief 函数模版 在node_handle中订阅topic,并与传入的回调函数进行注册
...
/**
 * @brief 声明ROS的一些topic的发布器, 服务的发布器, 以及将时间驱动的函数与定时器进行绑定
 * @param[in] node_options 配置文件的内容
 * @param[in] map_builder SLAM算法的具体实现
 * @param[in] tf_buffer tf
 * @param[in] collect_metrics 是否启用metrics,默认不启用  false
 */
Node::Node(const NodeOptions& node_options,
    ...
    // Step: 1 声明创建publisher对象
    // 发布SubmapList  构建好的子图列表
    ...
    // 发布landmark_pose  路标点位姿列表
    landmark_poses_list_publisher_ = node_handle_.advertise<::visualization_msgs::MarkerArray>(
        kLandmarkPosesListTopic, kLatestOnlyPublisherQueueSize);
    // 发布约束 优化约束
    constraint_list_publisher_ = node_handle_.advertise<::visualization_msgs::MarkerArray>(
        kConstraintListTopic, kLatestOnlyPublisherQueueSize);
    // 发布tracked_pose, 默认不发布  匹配的2D点云数据
    if (node_options_.publish_tracked_pose) {
        tracked_pose_publisher_ = node_handle_.advertise<::geometry_msgs::PoseStamped>(
            kTrackedPoseTopic, kLatestOnlyPublisherQueueSize);
    }
    // lx add 3d建图
    if (node_options_.map_builder_options.use_trajectory_builder_3d()) {
        point_cloud_map_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
            kPointCloudMapTopic, kLatestOnlyPublisherQueueSize, true);
    }

    // Step: 2 声明 service ROS服务, 并将服务的发布器放入到vector容器中
    service_servers_.push_back(
        node_handle_.advertiseService(kSubmapQueryServiceName, &Node::HandleSubmapQuery, this));
    service_servers_.push_back(node_handle_.advertiseService(kTrajectoryQueryServiceName,
                                                             &Node::HandleTrajectoryQuery, this));
    service_servers_.push_back(node_handle_.advertiseService(kStartTrajectoryServiceName,
                                                             &Node::HandleStartTrajectory, this));
    service_servers_.push_back(node_handle_.advertiseService(kFinishTrajectoryServiceName,
                                                             &Node::HandleFinishTrajectory, this));
    service_servers_.push_back(
        node_handle_.advertiseService(kWriteStateServiceName, &Node::HandleWriteState, this));
    service_servers_.push_back(node_handle_.advertiseService(
        kGetTrajectoryStatesServiceName, &Node::HandleGetTrajectoryStates, this));
    service_servers_.push_back(
        node_handle_.advertiseService(kReadMetricsServiceName, &Node::HandleReadMetrics, this));

    // Step: 3 处理之后的点云的发布器
    scan_matched_point_cloud_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
        kScanMatchedPointCloudTopic, kLatestOnlyPublisherQueueSize);

    // Step: 4 进行定时器与函数的绑定, 定时发布数据 与 step 1中创建的pub绑定
    ...
    wall_timers_.push_back(node_handle_.createWallTimer(
        ::ros::WallDuration(node_options_.trajectory_publish_period_sec), // 30e-3s
        &Node::PublishTrajectoryNodeList, this));
    wall_timers_.push_back(node_handle_.createWallTimer(
        ::ros::WallDuration(node_options_.trajectory_publish_period_sec), // 30e-3s
        &Node::PublishLandmarkPosesList, this));
    wall_timers_.push_back(
        node_handle_.createWallTimer(::ros::WallDuration(kConstraintPublishPeriodSec), // 0.5s
                                     &Node::PublishConstraintList, this));
    // lx add
    if (node_options_.map_builder_options.use_trajectory_builder_3d()) {
        wall_timers_.push_back(
            node_handle_.createWallTimer(::ros::WallDuration(kPointCloudMapPublishPeriodSec), // 10s
...
bool Node::HandleTrajectoryQuery(::cartographer_ros_msgs::TrajectoryQuery::Request& request,
    ...
    response.status = TrajectoryStateToStatus(request.trajectory_id,
    ...
    for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
        ...
        if (trajectory_data.local_slam_data->time != extrapolator.GetLastPoseTime()) {
            ...
            if (scan_matched_point_cloud_publisher_.getNumSubscribers() > 0) {
                ...
                point_cloud.reserve(
                    trajectory_data.local_slam_data->range_data_in_local.returns.size());
                ...
                for (const cartographer::sensor::RangefinderPoint point :
                    ...
                    point_cloud.push_back(
                        cartographer::sensor::ToTimedRangefinderPoint(point, 0.f /* time */));
                }
        ...
        stamped_transform.header.stamp = node_options_.use_pose_extrapolator
                                             ? ToRos(now)
...
void Node::LaunchSubscribers(const TrajectoryOptions& options, const int trajectory_id) {
    ...
    if (node_options_.map_builder_options.use_trajectory_builder_3d() ||
        (node_options_.map_builder_options.use_trajectory_builder_2d() &&
         options.trajectory_builder_options.trajectory_builder_2d_options().use_imu_data())) {
        subscribers_[trajectory_id].push_back(
            {SubscribeWithHandler<sensor_msgs::Imu>(&Node::HandleImuMessage, trajectory_id,
                                                    kImuTopic, &node_handle_, this),
             kImuTopic});
    }
    ...
    if (options.use_odometry) {
        subscribers_[trajectory_id].push_back(
            {SubscribeWithHandler<nav_msgs::Odometry>(&Node::HandleOdometryMessage, trajectory_id,
                                                      kOdometryTopic, &node_handle_, this),
             kOdometryTopic});
    }

    // gps 的订阅与注册回调函数,只有一个gps的topic
    if (options.use_nav_sat) {
        subscribers_[trajectory_id].push_back({SubscribeWithHandler<sensor_msgs::NavSatFix>(
                                                   &Node::HandleNavSatFixMessage, trajectory_id,
                                                   kNavSatFixTopic, &node_handle_, this),
                                               kNavSatFixTopic});
    }
    ...
    if (options.use_landmarks) {
        subscribers_[trajectory_id].push_back(
            {SubscribeWithHandler<cartographer_ros_msgs::LandmarkList>(
                 &Node::HandleLandmarkMessage, trajectory_id, kLandmarkTopic, &node_handle_, this),
             kLandmarkTopic});
    }
}
...
cartographer_ros_msgs::StatusResponse Node::TrajectoryStateToStatus(
    ...
    status_response.message = absl::StrCat("Trajectory ", trajectory_id, " is in '",
...
cartographer_ros_msgs::StatusResponse Node::FinishTrajectoryUnderLock(const int trajectory_id) {
    ...
    if (trajectories_scheduled_for_finish_.count(trajectory_id)) {
        status_response.message =
            absl::StrCat("Trajectory ", trajectory_id, " already pending to finish.");
    ...
    status_response =
        TrajectoryStateToStatus(trajectory_id, {TrajectoryState::ACTIVE} /* valid states */);
    // 如果不是OK状态就返回ERROR
 ...
 * @return true: ROS的service只能返回true, 返回false程序会中断
 */
bool Node::HandleStartTrajectory(::cartographer_ros_msgs::StartTrajectory::Request& request,
    ...
    if (request.use_initial_pose) {
        ...
        response.status = TrajectoryStateToStatus(request.relative_to_trajectory_id,
 ...
 * @return true
 */
bool Node::HandleFinishTrajectory(::cartographer_ros_msgs::FinishTrajectory::Request& request,
...
bool Node::HandleWriteState(::cartographer_ros_msgs::WriteState::Request& request,
                            ::cartographer_ros_msgs::WriteState::Response& response) {
    {
        ...
        if (map_builder_bridge_.SerializeState(request.filename,
            ...
            response.status.code = cartographer_ros_msgs::StatusCode::OK;
            response.status.message = absl::StrCat("State written to '", request.filename, "'.");
        } else {
            response.status.code = cartographer_ros_msgs::StatusCode::INVALID_ARGUMENT;
            response.status.message = absl::StrCat("Failed to write '", request.filename, "'.");
        }
    }
    // lx add
    ...
    if (node_options_.map_builder_options.use_trajectory_builder_3d() && save_pcd) {
        ...
        std::string prefix = request.filename.substr(0, request.filename.size() - suffix.size());
...
void Node::HandleImuMessage(const int trajectory_id, const std::string& sensor_id,
    ...
    // 除了要通过 map_builder_bridge_喂给建图器之外，还需要将IMU数据喂给位姿估计器
    auto sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id);
...
void Node::SerializeState(const std::string& filename, const bool include_unfinished_submaps) {
    absl::MutexLock lock(&mutex_);
    CHECK(map_builder_bridge_.SerializeState(filename, include_unfinished_submaps))
    ...
    for (const auto& entry : subscribers_) {
        ...
        for (const auto& subscriber : entry.second) {
            ...
            if (published_topics.count(resolved_topic) == 0) {
                LOG(WARNING) << "Expected topic \"" << subscriber.topic << "\" (trajectory "
                             << trajectory_id << ")" << " (resolved topic \"" << resolved_topic
    ...
    for (; node_it != end_it; ++node_it) {
        ...
        if (trajectory_node.constant_data != nullptr) {
            ...
            for (const RangefinderPoint& point : high_resolution_point_cloud.points()) {
                ...
                node_point_cloud->push_back(pcl::PointXYZ(range_finder_point.position.x(),
