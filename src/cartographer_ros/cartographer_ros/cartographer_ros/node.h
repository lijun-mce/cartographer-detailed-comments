...
public:
    /**
     * @brief 构造函数 声明ROS的一些topic的发布器, 服务的发布器,
     * 以及将时间驱动的函数与定时器进行绑定
...
private:
    ...
    bool HandleStartTrajectory(cartographer_ros_msgs::StartTrajectory::Request& request,
    ...
    cartographer_ros_msgs::StatusResponse FinishTrajectoryUnderLock(int trajectory_id)
    ...
    // 时间复杂度为常数级别O(1),而额外空间复杂度则要高出许多 对于需要高效率查询的情况,
    // 使用unordered_map容器, 但是unordered_map对于迭代器遍历效率并不高
