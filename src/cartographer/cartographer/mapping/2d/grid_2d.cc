...
Grid2D::Grid2D(const MapLimits& limits, float min_correspondence_cost,
               float max_correspondence_cost, ValueConversionTables* conversion_tables)
    : limits_(limits)
...
Grid2D::Grid2D(const proto::Grid2D& proto, ValueConversionTables* conversion_tables)
    ...
    if (proto.has_known_cells_box()) {
        ...
        known_cells_box_ = Eigen::AlignedBox2i(Eigen::Vector2i(box.min_x(), box.min_y()),
...
void Grid2D::GrowLimits(const Eigen::Vector2f& point,
    ...
    while (!limits_.Contains(limits_.GetCellIndex(point))) {
        ...
        const int new_size =
            new_limits.cell_limits().num_x_cells * new_limits.cell_limits().num_y_cells;
