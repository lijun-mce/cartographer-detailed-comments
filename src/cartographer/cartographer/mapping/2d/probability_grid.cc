...
// 如果单元格尚未更新,则将调用 ComputeLookupTableToApplyOdds() 时指定的 'odds' 应用于单元格在
// 'cell_index' 处的概率 在调用 FinishUpdate()
...
bool ProbabilityGrid::ApplyLookupTable(const Eigen::Array2i& cell_index,
    ...
    //  通过cell_index计算栅格单元的存储索引，获取对应的\(p_{free}\)存储值，并确保该值不会超出查找表的数组边界
...
GridType ProbabilityGrid::GetGridType() const { return GridType::PROBABILITY_GRID; }
...
std::unique_ptr<Grid2D> ProbabilityGrid::ComputeCroppedGrid() const {
    ...
    // 重新定义概率栅格地图的大小
    std::unique_ptr<ProbabilityGrid> cropped_grid = absl::make_unique<ProbabilityGrid>(
...
bool ProbabilityGrid::DrawToSubmapTexture(
    ...
    *texture->mutable_slice_pose() = transform::ToProto(
        local_pose.inverse() * transform::Rigid3d::Translation(Eigen::Vector3d(max_x, max_y, 0.)));
