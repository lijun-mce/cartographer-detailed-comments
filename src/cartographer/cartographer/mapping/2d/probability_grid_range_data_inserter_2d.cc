 ...
 */
void CastRays(const sensor::RangeData& range_data, const std::vector<uint16>& hit_table,
              const std::vector<uint16>& miss_table, const bool insert_free_space,
...
proto::ProbabilityGridRangeDataInserterOptions2D CreateProbabilityGridRangeDataInserterOptions2D(
    ...
    options.set_insert_free_space(parameter_dictionary->HasKey("insert_free_space")
...
void ProbabilityGridRangeDataInserter2D::Insert(const sensor::RangeData& range_data,
                                                GridInterface* const grid) const {
