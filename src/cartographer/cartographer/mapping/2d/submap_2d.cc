...
proto::SubmapsOptions2D CreateSubmapsOptions2D(
    ...
    *options.mutable_range_data_inserter_options() = CreateRangeDataInserterOptions(
    ...
    CHECK(valid_range_data_inserter_grid_combination)
        << "Invalid combination grid_type " << grid_type << " with range_data_inserter_type "
...
Submap2D::Submap2D(const proto::Submap2D& proto, ValueConversionTables* conversion_tables)
    : Submap(transform::ToRigid3(proto.local_pose())), conversion_tables_(conversion_tables) {
    if (proto.has_grid()) {
...
void Submap2D::UpdateFromProto(const proto::Submap& proto) {
    ...
    if (proto.submap_2d().has_grid()) {
        if (proto.submap_2d().grid().has_probability_grid_2d()) {
            grid_ =
                absl::make_unique<ProbabilityGrid>(proto.submap_2d().grid(), conversion_tables_);
        } else if (proto.submap_2d().grid().has_tsdf_2d()) {
            grid_ = absl::make_unique<TSDF2D>(proto.submap_2d().grid(), conversion_tables_);
        } else {
            LOG(FATAL) << "proto::Submap2D has grid with unknown type.";
        }
    }
 ...
 */
void Submap2D::ToResponseProto(const transform::Rigid3d&,
...
std::unique_ptr<GridInterface> ActiveSubmaps2D::CreateGrid(const Eigen::Vector2f& origin) {
    // 地图初始大小,100个栅格  继承关系 GridInterface --> Grid2D --> ProbabilityGrid
...
// 新增一个子图,根据子图个数判断是否删掉第一个子图  // origin 新建子图的原点坐标
void ActiveSubmaps2D::AddSubmap(const Eigen::Vector2f& origin) {
    ...
    submaps_.push_back(absl::make_unique<Submap2D>(
        origin, std::unique_ptr<Grid2D>(static_cast<Grid2D*>(CreateGrid(origin).release())),
        &conversion_tables_));
}
