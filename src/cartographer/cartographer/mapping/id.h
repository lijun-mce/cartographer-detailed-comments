...
struct NodeId {
    ...
    // 一条轨迹由若干个节点串联起来的一个数据结构
    int trajectory_id; // 记录了轨迹的索引
    int node_index;    // 从零开始计数的节点编号
...
public:
    ...
    public:
        ...
        IdDataReference operator*() const {
            ...
            return IdDataReference{IdType{current_trajectory_->first, current_data_->first},
    ...
    ConstIterator find(const IdType& id) const { return ConstIterator(*this, id); }
    ...
    size_t SizeOfTrajectoryOrZero(const int trajectory_id) const {
        return trajectories_.count(trajectory_id) ? trajectories_.at(trajectory_id).data_.size()
                                                  : 0;
    }
    ...
    // 整个表的最后位置的迭代器
    ConstIterator end() const { return BeginOfTrajectory(std::numeric_limits<int>::max()); }
    // 整个表是否是空的
