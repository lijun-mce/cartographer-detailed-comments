...
void MaybeAddPureLocalizationTrimmer(const int trajectory_id,
    ...
    if (trajectory_options.pure_localization()) {
        ...
        pose_graph->AddTrimmer(
            absl::make_unique<PureLocalizationTrimmer>(trajectory_id, 3 /* max_submaps_to_keep */));
        return;
    }
...
MapBuilder::MapBuilder(const proto::MapBuilderOptions& options)
    : options_(options), thread_pool_(options.num_background_threads()) { // 创建线程的数量
    // 确保在2D和3D建图中二选一
...
int MapBuilder::AddTrajectoryBuilder(const std::set<SensorId>& expected_sensor_ids,
    ...
    // id从 0 开始的, 所以新trajectory_id就是trajectory_builders_的size()
    ...
    if (options_.use_trajectory_builder_3d()) {
         ...
         * 该运算符把expression转换为type-id类型,
         但没有运行时类型检查来保证转换的安全性 （1）用于基本数据类型之间的转换,
         如把int转换为char, 把int转换成enum, （2）把空指针转换成目标类型的空指针
          （3）把任何类型的表达式类型转换成void类型
          （4）用于类层次结构中父类和子类之间指针和引用的转换.

         * c++11: dynamic_cast关键字（运行时类型检查）: dynamic_cast < type-id > (
         expression ) 该运算符把 expression 转换成 type-id 类型的对象.
        ...
        trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
            trajectory_options, sensor_collator_.get(), trajectory_id, expected_sensor_ids,
            // 将3D前端与3D位姿图打包在一起, 传入CollatedTrajectoryBuilder
            CreateGlobalTrajectoryBuilder3D(std::move(local_trajectory_builder), trajectory_id,
    ...
    // 2d的轨迹
        ...
        // 几乎完成了一个局部SLAM的所有功能，包括位姿估计、扫描匹配
        ...
        // 将 pose_graph_ 强制转化为 PoseGraph2D 并进行检查 （后端）
        DCHECK(dynamic_cast<PoseGraph2D*>(pose_graph_.get()));

        // CollatedTrajectoryBuilder 理解为完整的地图构建器
        // local_trajectory_builder 局部地图构建器（前端）
        // PoseGraph2D（后端）
        trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
            trajectory_options, sensor_collator_.get(), trajectory_id, expected_sensor_ids,
            // 将2D前端与2D位姿图后端打包在一起, 传入CollatedTrajectoryBuilder
            // 应该是一个具有闭环检测功能的slam对象
    ...
    // 如果给了初始位姿，那么我们可以将初始位置提供给pose_graph_对象
...
std::string MapBuilder::SubmapToProto(const SubmapId& submap_id,
    ...
    // 进行id的检查
    if (submap_id.trajectory_id < 0 || submap_id.trajectory_id >= num_trajectory_builders()) {
        return "Requested submap from trajectory " + std::to_string(submap_id.trajectory_id) +
    ...
    if (submap_data.submap == nullptr) {
        return "Requested submap " + std::to_string(submap_id.submap_index) + " from trajectory " +
               std::to_string(submap_id.trajectory_id) +
...
std::map<int, int> MapBuilder::LoadState(io::ProtoStreamReaderInterface* const reader,
    ...
    for (int i = 0; i < pose_graph_proto.trajectory_size(); ++i) {
        ...
        CHECK(trajectory_remapping.emplace(trajectory_proto.trajectory_id(), new_trajectory_id)
                  .second)
    ...
    if (options_.use_trajectory_builder_3d()) {
        CHECK_NE(deserializer.header().format_version(), io::kFormatVersionWithoutSubmapHistograms)
    ...
    while (deserializer.ReadNextSerializedData(&proto)) {
        switch (proto.data_case()) {
            ...
            case SerializedData::kSubmap: {
                ...
                // 将submap添加到位姿图中
                pose_graph_->AddSubmapFromProto(submap_poses.at(submap_id), proto.submap());
                break;
            }
...
std::map<int, int> MapBuilder::LoadStateFromFile(const std::string& state_filename,
    ...
    if (state_filename.substr(std::max<int>(state_filename.size() - suffix.size(), 0)) != suffix) {
...
// 工厂函数（利用多态实现）
// MapBuilderInterface 是 MapBuilder的一个基类（父类的指针或引用构造子类对象）
