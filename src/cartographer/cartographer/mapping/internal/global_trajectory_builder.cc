...
public:
    ...
    void AddSensorData(const std::string& sensor_id,
        ...
        if (matching_result->insertion_result != nullptr) {
            ...
            // 将匹配后的结果 当做节点 加入到位姿图中 把前端的结果喂给后端，获取当前轨迹节点的索引
            ...
            insertion_result = absl::make_unique<InsertionResult>(
                InsertionResult{node_id, matching_result->insertion_result->constant_data,
        ...
        if (local_slam_result_callback_) {
            // 回调调用 map_builder_bridge.cc 文件中的 OnLocalSlamResult函数
    ...
    // 一个是进入后端pose_graph_ 加入到后端之前, 先做一个距离的计算, 只有时间,移动距离,角度
    // 变换量大于阈值才加入到后端中
    void AddSensorData(const std::string& sensor_id,
        ...
        if (pose_graph_odometry_motion_filter_.has_value() &&
            pose_graph_odometry_motion_filter_.value().IsSimilar(odometry_data.time,
                                                                 odometry_data.pose)) {
            return;
        }
    ...
    void AddLocalSlamResultData(
        std::unique_ptr<mapping::LocalSlamResultData> local_slam_result_data) override {
...
private:
    ...
    // 模板参数, 可以指向PoseGraph2D也可以指向PoseGraph3D
    PoseGraph* const pose_graph_;
    // 位姿跟踪器，前端的核心对象，其数据类型是一个模板参数
...
/**
 * @brief 连接前端和后端的其桥梁  2d的完整的slam
 * @param local_trajectory_builder 位姿跟踪器，前端的核心对象
 * @param trajectory_id 轨迹索引
 * @param pose_graph 位姿图，后端的核心对象
...
void GlobalTrajectoryBuilderRegisterMetrics(metrics::FamilyFactory* factory) {
    auto* results = factory->NewCounterFamily(
        "mapping_global_trajectory_builder_local_slam_results", "Local SLAM results");
