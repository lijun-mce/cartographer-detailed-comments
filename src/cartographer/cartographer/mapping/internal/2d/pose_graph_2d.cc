...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
        ...
        if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
            ...
            if (data_.initial_trajectory_poses.count(trajectory_id) > 0) {
                ...
                data_.trajectory_connectivity_state.Connect(
                    trajectory_id,
                    data_.initial_trajectory_poses.at(trajectory_id).to_trajectory_id, time);
            }
    ...
    if (data_.submap_data.at(last_submap_id).submap == insertion_submaps.front()) {
        ...
        optimization_problem_->AddSubmap(
            ...
            // globla指向local的坐标变换 globla指向local的坐标变换 * 第二个子图原点在local下的坐标 =
            // 第二个子图原点在global下的坐标
            first_submap_pose * constraints::ComputeSubmapPose(*insertion_submaps[0]).inverse() *
...
NodeId PoseGraph2D::AppendNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data, const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps,
    ...
    const NodeId node_id =
        data_.trajectory_nodes.Append(trajectory_id, TrajectoryNode{constant_data, optimized_pose});
    // 节点总个数加1
    ...
    if (data_.submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0 ||
        std::prev(data_.submap_data.EndOfTrajectory(trajectory_id))->data.submap !=
        ...
        const SubmapId submap_id = data_.submap_data.Append(trajectory_id, InternalSubmapData());
...
NodeId PoseGraph2D::AddNode(std::shared_ptr<const TrajectoryNode::Data> constant_data,
    ...
    const transform::Rigid3d optimized_pose(GetLocalToGlobalTransform(trajectory_id) *
                                            constant_data->local_pose);

    // 向节点列表加入节点,并得到节点的id
    const NodeId node_id =
        AppendNode(constant_data, trajectory_id, insertion_submaps, optimized_pose);
...
void PoseGraph2D::AddWorkItem(const std::function<WorkItem::Result()>& work_item) {
    ...
    kWorkQueueDelayMetric->Set(
        std::chrono::duration_cast<std::chrono::duration<double>>(now - work_queue_->front().time)
            .count());
}
...
void PoseGraph2D::AddFixedFramePoseData(const int trajectory_id,
            ...
            for (const auto& observation : landmark_data.landmark_observations) {
                data_.landmark_nodes[observation.id].landmark_observations.emplace_back(
                    PoseGraphInterface::LandmarkNode::LandmarkObservation{
                        ...
                        observation.landmark_to_tracking_transform, observation.translation_weight,
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id, const SubmapId& submap_id) {
...

    {
        ...
        const common::Time last_connection_time =
            data_.trajectory_connectivity_state.LastConnectionTime(node_id.trajectory_id,
    ...
    if (maybe_add_local_constraint) {
        ...
        constraint_builder_.MaybeAddConstraint(submap_id, submap, node_id, constant_data,
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
    const NodeId& node_id, std::vector<std::shared_ptr<const Submap2D>> insertion_submaps,
    ...
    // 保存节点与计算子图内约束
    {
        ...
        submap_ids = InitializeGlobalSubmapPoses(node_id.trajectory_id, constant_data->time,
                                                 insertion_submaps);
        ...
        const transform::Rigid2d local_pose_2d = transform::Project2D(
            constant_data->local_pose * // 三维转平面
        ...
        const transform::Rigid2d global_pose_2d =
            ...
            constraints::ComputeSubmapPose(*insertion_submaps.front()).inverse() * local_pose_2d;

        // 把该节点的信息加入到OptimizationProblem中
        optimization_problem_->AddTrajectoryNode(
            matching_id.trajectory_id,
            optimization::NodeSpec2D{constant_data->time, local_pose_2d, global_pose_2d,
...
void PoseGraph2D::UpdateTrajectoryConnectivity(const Constraint& constraint) {
    ...
    data_.trajectory_connectivity_state.Connect(constraint.node_id.trajectory_id,
        ...
        std::map<int, SubmapId> trajectory_id_to_last_optimized_submap_id;
        {
            ...
            for (const int trajectory_id : node_data.trajectory_ids()) {
                ...
                trajectory_id_to_last_optimized_node_id.emplace(
                    trajectory_id, std::prev(node_data.EndOfTrajectory(trajectory_id))->id);
                trajectory_id_to_last_optimized_submap_id.emplace(
                    trajectory_id, std::prev(submap_data.EndOfTrajectory(trajectory_id))->id);
            }
        }
...
void PoseGraph2D::DrainWorkQueue() {
    ...
    constraint_builder_.WhenDone([this](const constraints::ConstraintBuilder2D::Result& result) {
...
void PoseGraph2D::WaitForAllComputations() {
    ...
    constraint_builder_.WhenDone(
        [this, &notification](const constraints::ConstraintBuilder2D::Result& result)
...
void PoseGraph2D::FreezeTrajectory(const int trajectory_id) {
    ...
    AddWorkItem([this, trajectory_id]() LOCKS_EXCLUDED(mutex_) {
        ...
        for (const auto& entry : data_.trajectories_state) {
            ...
            data_.trajectory_connectivity_state.Connect(trajectory_id, other_trajectory_id,
...
// 从proto流数据中添加Submap
void PoseGraph2D::AddSubmapFromProto(const transform::Rigid3d& global_submap_pose,
    ...
    const transform::Rigid2d global_submap_pose_2d = transform::Project2D(global_submap_pose);
    {
        ...
        data_.global_submap_poses_2d.Insert(submap_id,
                                            optimization::SubmapSpec2D{global_submap_pose_2d});
    }
...
void PoseGraph2D::AddNodeFromProto(const transform::Rigid3d& global_pose, const proto::Node& node) {
    ...
    AddWorkItem([this, node_id, global_pose]() LOCKS_EXCLUDED(mutex_) {
        absl::MutexLock locker(&mutex_);
        const auto& constant_data = data_.trajectory_nodes.at(node_id).constant_data;
        const auto gravity_alignment_inverse =
            transform::Rigid3d::Rotation(constant_data->gravity_alignment.inverse());
        optimization_problem_->InsertTrajectoryNode(
            node_id,
            optimization::NodeSpec2D{
                constant_data->time,
                transform::Project2D(constant_data->local_pose * gravity_alignment_inverse),
        ...
        for (const auto& constraint : constraints) {
            ...
            const Constraint::Pose pose = {
                constraint.pose.zbar_ij *
                    transform::Rigid3d::Rotation(data_.trajectory_nodes.at(constraint.node_id)
                                                     .constant_data->gravity_alignment.inverse()),
                constraint.pose.translation_weight, constraint.pose.rotation_weight};
            data_.constraints.push_back(
                Constraint{constraint.submap_id, constraint.node_id, pose, constraint.tag});
        }
...
void PoseGraph2D::RunFinalOptimization() {
    {
        ...
        AddWorkItem([this]() LOCKS_EXCLUDED(mutex_) {
            absl::MutexLock locker(&mutex_);
            // 再设置回去
            optimization_problem_->SetMaxNumIterations(options_.optimization_problem_options()
    ...
    for (const auto& node_id_data : data_.trajectory_nodes) {
        ...
        if (node_id_data.data.constant_data != nullptr) {
            constant_pose_data = TrajectoryNodePose::ConstantPoseData{
                node_id_data.data.constant_data->time, node_id_data.data.constant_data->local_pose};
        }
        node_poses.Insert(node_id_data.id,
...
void PoseGraph2D::SetLandmarkPose(const std::string& landmark_id,
                                  const transform::Rigid3d& global_pose, const bool frozen) {
    AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        absl::MutexLock locker(&mutex_);
...
transform::Rigid3d PoseGraph2D::GetInterpolatedGlobalTrajectoryPose(const int trajectory_id,
...
MapById<SubmapId, PoseGraphInterface::SubmapPose> PoseGraph2D::GetAllSubmapPoses() const {
    ...
    for (const auto& submap_id_data : data_.submap_data) {
        ...
        submap_poses.Insert(
            submap_id_data.id,
            PoseGraph::SubmapPose{submap_data.submap->num_range_data(), submap_data.pose});
    }
    return submap_poses;
}
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
    ...
    // global_pose->local_pose的坐标变换 tag: 画图说明一下
    return transform::Embed3D(global_submap_poses.at(last_optimized_submap_id).global_pose) *
           data_.submap_data.at(last_optimized_submap_id).submap->local_pose().inverse();
}
...
PoseGraphInterface::SubmapData PoseGraph2D::GetSubmapDataUnderLock(
    ...
    return {submap,
            ComputeLocalToGlobalTransform(data_.global_submap_poses_2d, submap_id.trajectory_id) *
                submap->local_pose()};
}

/************ PoseGraph2D::TrimmingHandle ***********************/

PoseGraph2D::TrimmingHandle::TrimmingHandle(PoseGraph2D* const parent) : parent_(parent) {}
    ...
    for (const auto& submap_id_data : parent_->data_.submap_data) {
        ...
        submaps.Insert(
            ...
            SubmapData{
                ...
                transform::Embed3D(
                    parent_->data_.global_submap_poses_2d.at(submap_id_data.id).global_pose)});
    }
    return submaps;
}
...
const std::vector<PoseGraphInterface::Constraint>& PoseGraph2D::TrimmingHandle::GetConstraints()
    const {
...
void PoseGraph2D::RegisterMetrics(metrics::FamilyFactory* family_factory) {
    auto* latency =
        family_factory->NewGaugeFamily("mapping_2d_pose_graph_work_queue_delay",
                                       "Age of the oldest entry in the work queue in seconds");
    kWorkQueueDelayMetric = latency->Add({});
    auto* queue_size = family_factory->NewGaugeFamily("mapping_2d_pose_graph_work_queue_size",
                                                      "Number of items in the work queue");
    kWorkQueueSizeMetric = queue_size->Add({});
    auto* constraints = family_factory->NewGaugeFamily(
        "mapping_2d_pose_graph_constraints", "Current number of constraints in the pose graph");
    ...
    auto* submaps = family_factory->NewGaugeFamily("mapping_2d_pose_graph_submaps",
